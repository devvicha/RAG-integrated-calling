...
export default function ToolEditorModal({
  ...
  const handleSave = () => {
    ...
    try {
      console.log('Attempting to parse parametersStr:', parametersStr);

      if (typeof parametersStr !== 'string' || parametersStr.trim() === '') {
        throw new Error('parametersStr is not a valid JSON string.');
      }

      // Ensure parametersStr is a valid JSON string
      if (!parametersStr.startsWith('{') || !parametersStr.endsWith('}')) {
        throw new Error('parametersStr must be a JSON object string.');
      }

      parsedParameters = JSON.parse(parametersStr);
      setJsonError(null);
    } catch (error) {
      console.error('Error parsing parametersStr:', error);
